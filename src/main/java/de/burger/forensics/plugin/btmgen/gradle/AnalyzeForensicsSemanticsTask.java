...
public abstract class AnalyzeForensicsSemanticsTask extends DefaultTask {
    ...
    public void analyze() {
        ...
        try {
            new SemanticEnrichmentRunner(this::semanticAnalysisPort, this::analysisStore)
                    .analyze(new SemanticEnrichmentRequest(
                            sourceRootPaths(),
                            joernExecutable("joern"),
                            joernExecutable("joern-parse"),
                            joernExecutable("joern-slice"),
                            getJoernWorkspaceDirectory().get().getAsFile().toPath(),
                            ...
                            getManifestFile().get().getAsFile().toPath(),
                            getChecksumsFile().get().getAsFile().toPath(),
                            getOutputFile().get().getAsFile().toPath()));
        } catch (BtmGenerationException | UncheckedIOException | IllegalArgumentException exception) {
