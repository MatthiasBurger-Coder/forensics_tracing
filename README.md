...
This repository hosts the `de.burger.forensics.btmgen` Gradle plugin. The plugin analyses Java sources and produces Byteman rules that capture forensic traces of call-chains, decision points, and configured variable writes. The generated Byteman script can be injected into a JVM to obtain a "glass trail" of how a function executed across the codebase.

## Features

- Generates `AT ENTRY` / `AT EXIT` rules for methods to trace call-chains.
- Adds decision tracing for Java `if`/`else-if` chains and `switch` statements, including true/false branches and individual cases.
- Emits `AFTER WRITE` rules for user-selected local variables when using the legacy regex pipeline.
- Provides an opt-in naive Java parser for additional coverage.
- Skips unsupported languages automatically; Kotlin `.kt` / `.kts` files are ignored with a warning.
...
1. **Verify prerequisites**
   ...
   - Java source files in your project (Kotlin sources are ignored during scanning).
2. **Add the plugin to your project** – extend the `plugins` block in `build.gradle`:
...
tasks.register("generateBtmRules", de.burger.forensics.plugin.GenerateBtmTask) {
    ...
    includePatterns.set([])
    excludePatterns.set([])
    parallelism.set(Math.max(Runtime.getRuntime().availableProcessors(), 1))
    shards.set(Math.max(Runtime.getRuntime().availableProcessors(), 1))
